...
impl DatabaseClient {
    ...
    pub async fn select(
        ...
        let clause_publisher: Option<String> = publisher.map(|p| format!("publisher = '{}'", p));
        let clause_lang: Option<String> = lang.map(|l| format!("lang = '{}'", l));
        let search_clause: Option<String> = search_term.map(|t| {
            format!(
                "author LIKE '%{}%' OR title LIKE '%{}%' OR link LIKE '%{}%'",
                t, t, t
            )
        });

        let clause: String = match clause_publisher.or(clause_lang) {
            Some(enumeration) => {
                ...
                match (author, title, link, publisher, lang) {
                    (Some(author), Some(title), Some(link), Some(publisher), Some(lang)) => {
                        match (Publisher::from(&publisher), Lang::from(&lang)) {
                            (Some(publisher), Some(lang)) => Ok(RSSFeed {
                                author,
